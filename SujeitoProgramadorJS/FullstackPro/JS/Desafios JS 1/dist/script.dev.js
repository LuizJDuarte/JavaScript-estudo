...
function numberVerify(number) {
  console.log("O n\xFAmero escolhido foi: ".concat(number));
...
numberVerify(parseInt(prompt("Digite um número para saber se ele é positivo, negativo ou zero: "))); //2) Desafio da Verificação de Elemento em um Array: Crie uma função que deve esperar receber 2 parâmetros, primeiro deve ser um array de números 
...
function numberArray(array, numero) {
  var arrayF = array.find(function (item) {
    if (item === numero) {
      console.log("Em cheio! Na lista cont\xE9m o n\xFAmero: ".concat(item));
    }
  });
}

numberArray([1, 2, 3, 4, 5, 6, 7, 8], 1); //3) Com base no array abaixo o desafio como você pode percorrer e encontrar o produto com Preço Igual a R$20 e o resultado deve mostrar apenas 
...
function acharProdutoPreco() {
  ...
  produtos.map(function (item) {
    if (item.price === 20) {
      console.log("Achamos o ".concat(item.price));
    }
  });
}

acharProdutoPreco(); //3.1) Com o mesmo array de produtos acima como você pode Filtrar Produtos com Preço Menor que R$8 ?

function filtrarPrice() {
  var produtos = products.filter(function (item) {
    return item.price < 8;
  });
